...
class Aspek_operasional_model extends CI_Model {
	...
	function get_peserta_aktif_cabang_semester($semester=null){
        $tahun = $this->session->userdata('tahun');
		$iduser = $this->session->userdata('iduser');
		...
		if($semester == null){
			...
			$sql ="
				...
				LEFT JOIN  mst_kelompok_peserta_aktif b USING (id_kelompok,iduser)
				LEFT JOIN mst_cabang c USING (id_cabang,iduser)
				WHERE a.iduser = ? AND a.tahun = ? and a.semester = ?
			";
			return $this->db->query($sql,[$iduser,$tahun,$semester])->result_array();
		}
