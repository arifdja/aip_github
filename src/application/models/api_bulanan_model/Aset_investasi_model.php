...
class Aset_investasi_model extends CI_Model {
      ...
      if (in_array($id_investasi, $arrID) && in_array($id_user, $arrUSER) && in_array($id_bulan, $arrBulan)) 
      {
        ...
        if($status_input == true)
        {
          ...
          if(in_array($id_investasi,$arr_group[1])){
            ...
            //saldo akhir = saldo awal + pembelian - penjualan + diskonto/premium + kenaikan/penurunan harga pasar
            $return = $this->validasi_form_2($id_investasi,$key,$data,$detail,$id_user);
            if($return){
              $status = 0;
              $res=array();
              $res['error']=true;
          ...
          if ($cekdata>0) {
            ...
            $gelondongan = $this->investasi_gelondongan($id_investasi,$id_user);
            if(!$gelondongan){
              ...
              $dataUpdateValid['realisasi_rka'] = $dataUpdate['realisasi_rka'];
              $dataUpdateValid['rka'] = $dataUpdate['rka'];
            ...
            if(!$gelondongan){
              ...
              $dataInsertValid['realisasi_rka']= $dataInsert['realisasi_rka'];
              $dataInsertValid['rka']= $dataInsert['rka'];
   ...
   * Saldo akhir = Saldo awal + pembelian - penjualan + diskonto/premium + kenaikan/penurunan harga pasar
   */
  private function validasi_form_2($id_investasi,$key,$data,$detail,$id_user)
  {
  ...
  /**
   * Validasi form investasi_gelondongan
   * 
   * @param int $id_investasi
   * @param int $iduser
   * @return string
   */
  private function investasi_gelondongan($id_investasi,$iduser)
  {
    // var_dump($id_investasi);exit;
    $header_reksadana = $this->db->get_where('mst_investasi',array('iduser'=>$iduser,'jenis_investasi'=>"Reksadana",'group'=>'INVESTASI'))->row_array();
    // var_dump($header_reksadana);exit;
    
    if ($header_reksadana) {
        $list_anak_reksadana = $this->db->get_where('mst_investasi',array('iduser'=>$iduser,'parent_id'=>$header_reksadana['id_investasi'],'group'=>'INVESTASI'))->result_array();
        $id_gelondongan = [];
        foreach ($list_anak_reksadana as $row) {
          $id_gelondongan[] = $row['id_investasi'];
        }

        if(in_array($id_investasi,$id_gelondongan)){
