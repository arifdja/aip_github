...
class Aspek_investasi extends CI_Controller {
    ...
    public function aset_investasi_semester(){
        ...
        foreach ($invest as $k => $v) {
            ...
            $array[$k]['child'] = array();
            if($v['type'] == "PC"){
                $child = $this->aspek_investasi_model->getdataindex('aset_investasi_front_lv2','result_array', $v['id_investasi'], $param_jenis);
                foreach ($child as $key => $val) {
                    ...
                    if($val['type'] == "PC"){
                        $childinvest = $this->aspek_investasi_model->getdataindex('aset_investasi_front_lv3','result_array', $val['id_investasi'], $param_jenis);
                        foreach ($childinvest as $x => $y) {
                            ...
                            $array[$k]['child'][$key]['subchild'][$x]['nominal'] = $y['saldo_akhir_smt2'] - $y['saldo_akhir_smt1'];

                            $pers_nom2 = ($y['saldo_akhir_smt1']!=0)?(($y['saldo_akhir_smt2'] - $y['saldo_akhir_smt1'])/$y['saldo_akhir_smt1'])*100:0;
                            $pers_nom2 = (!is_nan($pers_nom2) && !is_infinite($pers_nom2) ? $pers_nom2 : '0,00');
                ...
                foreach ($child as $key => $val) {
                    ...
                    if($val['type'] == "PC"){
                        ...
                        foreach ($childinvest as $x => $y) {
                           ...
                           $pers_rka2= ($y['rka']!=0)?($y['saldo_akhir_smt2']/$y['rka'])*100:0;
                           $pers_rka2 = (!is_nan($pers_rka2) && !is_infinite($pers_rka2) ? $pers_rka2 : '0,00');
                           $array[$k]['child'][$key]['subchild'][$x]['pers_rka'] = $pers_rka2;
                           $array[$k]['child'][$key]['subchild'][$x]['nominal'] = $y['saldo_akhir_smt2'] - $y['saldo_akhir_smt1'];
                           $pers_nom2 = ($y['saldo_akhir_smt1']!=0)?(($y['saldo_akhir_smt2'] - $y['saldo_akhir_smt1'])/$y['saldo_akhir_smt1'])*100:0;
    ...
    public function hasil_investasi_semester_bkp(){
        $param_jenis = 'HASIL INVESTASI';
        $array = array();
